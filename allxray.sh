...
read_and_validate_port() {
    read -p "请输入 $1 端口号：" port
    while ! echo "$port" | grep -qE '^[0-9]+$'; do
        print_error "错误：端口号必须是数字!"
        read -p "请重新输入 $1 端口号：" port
    done
    while [ "$port" -lt 1 ] || [ "$port" -gt 65535 ]; do
        print_error "错误：端口号必须介于1~65535之间!"
        read -p "请重新输入 $1 端口号：" port
    done
    while ! nc -z 127.0.0.1 "$port" 2>/dev/null; do
        print_error "错误：$port 已被占用！"
        read -p "请重新输入 $1 端口号：" port
    done
    print_green "成功：端口号 $port 可用!"
}

read_and_validate_port "reality"
